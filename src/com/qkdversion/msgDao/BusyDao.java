...
public class BusyDao {
	...
	// 检查链路占用情况
	public boolean isLinkBusy(ArrayList<Msg> msgList,
			ArrayList<AckMsg> linkList, Msg msg) {
	...
	private boolean upPath(ArrayList<Msg> msgList, ArrayList<AckMsg> linkList,
			Msg msg) {
		...
		case MsgConstant.FIBER:
			...
			if (msg.getFiberIp() == MsgConstant.IP1) {// 如果是到目的IP1
				if (fiberSet.contains("33") || fiberSet.contains("23")
						|| fiberSet.contains("14") || fiberSet.contains("11")) {
					return true;
				}
	...
	private boolean downPath(ArrayList<Msg> msgList,
			ArrayList<AckMsg> linkList, Msg msg) {
		int type = msg.getSwitchType();
		Set<String> fiberSet = getFiberAck(linkList);
		switch (type) {
		case MsgConstant.FIBER:
			...
			if (msg.getFiberIp() == MsgConstant.IP1) {// 如果是到目的IP1
				...
				if (fiberSet.contains("14") || fiberSet.contains("44")
						|| fiberSet.contains("22") || fiberSet.contains("23")) {
					return true;
				}
			}
			break;
