...
public final class MsgDao {
	 ...
	 * 封装波长级分配结果
	 **/
	public AckMsg makeLengthAck(int lengthType, int ack, Msg msg,
			List<AckMsg> list) {
		AckMsg ackMsg = new AckMsg();
	...
	public void bandChoose1(Msg msg, AckMsg ackMsg) {
		...
		if (msg.getFlag() == MsgConstant.ONE) {
			for (UserMsg user : userList) {
				if (user.getIpDes1().equals(MsgConstant.IP1)
						&& user.getIpDes2().equals("0")) {
					flag1 = true;
				}
				if (user.getIpDes1().equals(MsgConstant.IP2)
						&& user.getIpDes2().equals("0")) {
					flag2 = true;
				}
			}
